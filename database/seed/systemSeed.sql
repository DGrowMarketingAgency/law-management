-- =======================================================
-- Legal Practice Management Platform - System Seed Data
-- System Roles, System Permissions, and Role-Permission Matrix
-- (Strictly NO user accounts or business demo records)
-- =======================================================

USE `legal_practice`;

-- 1. Insert Standard System Roles
...
('CLIENT', 'Client external portal account with view-only access to own matters and bills'),
...
-- 2. Insert Standard System Permissions
...
('EMAIL_VIEW', 'EMAIL', 'Inspect chambers outgoing email logs and transmission status'),
('EMAIL_TEST', 'EMAIL', 'Perform SMTP connection test from dashboard'),
('EMAIL_TEMPLATE_VIEW', 'EMAIL', 'View legal notification email templates'),
('EMAIL_TEMPLATE_UPDATE', 'EMAIL', 'Modify legal notification email templates')
ON DUPLICATE KEY UPDATE `description` = VALUES(`description`);
...
-- B. ADMIN gets global IT and user permissions
INSERT IGNORE INTO `role_permissions` (`role_id`, `permission_id`)
SELECT r.id, p.id
FROM `roles` r
JOIN `permissions` p ON p.name IN (
  'USER_VIEW', 'USER_CREATE', 'USER_UPDATE', 'USER_DISABLE', 'USER_ROLE_UPDATE',
  'AUDIT_VIEW', 'EMAIL_VIEW', 'EMAIL_TEST', 'EMAIL_TEMPLATE_VIEW', 'EMAIL_TEMPLATE_UPDATE'
)
WHERE r.name = 'ADMIN';

-- C. SENIOR_ASSOCIATE permissions
INSERT IGNORE INTO `role_permissions` (`role_id`, `permission_id`)
SELECT r.id, p.id
FROM `roles` r
JOIN `permissions` p ON p.name IN (
...
-- D. JUNIOR_ASSOCIATE permissions
INSERT IGNORE INTO `role_permissions` (`role_id`, `permission_id`)
SELECT r.id, p.id
FROM `roles` r
JOIN `permissions` p ON p.name IN (
